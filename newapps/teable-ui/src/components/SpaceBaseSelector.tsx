...
// 拆分后的独立选择器：仅 Space 下拉
export const SpaceSelect = ({
  spaces,
  spaceId,
  onChange,
}: Pick<SpaceBaseSelectorProps, "spaces"> & { spaceId?: string; onChange: (spaceId: string) => void }) => {
  const currentSpace = useMemo(() => spaces.find((s) => s.id === spaceId) ?? spaces[0], [spaces, spaceId]);
  return (
    <Select
      value={spaceId ?? currentSpace?.id}
      onValueChange={(newSpaceId) => onChange(newSpaceId)}
    >
      <SelectTrigger className="w-44 h-8 bg-obsidian-bg border-obsidian-border text-obsidian-text">
        <SelectValue placeholder="选择 Space" />
      </SelectTrigger>
...
export const BaseSelect = ({
  ...
  baseId,
  onChange,
}: { bases: Array<{ id: string; name: string; tables: string[] }>; baseId?: string; onChange: (baseId: string) => void }) => {
  const first = bases?.[0]?.id;
  return (
    <Select value={baseId ?? first} onValueChange={(newBaseId) => onChange(newBaseId)}>
      <SelectTrigger className="w-44 h-8 bg-obsidian-bg border-obsidian-border text-obsidian-text">
        <SelectValue placeholder="选择 Base" />
      </SelectTrigger>
      <SelectContent className="bg-obsidian-surface border-obsidian-border text-obsidian-text">
